...
public class BookController extends BaseController {
    ...
    public void getChapterContent(HttpServletResponse response, HttpServletRequest request) {
        ...
        try{
            Chapter chapter = this.chapterService.getChapterById(Long.parseLong(chapterId),1);
    ...
    public void buyChapter(HttpServletResponse response, HttpServletRequest request) {
        ...
        if(StringUtils.isBlank(chapterId) || StringUtils.isBlank(userId)){
            logger.error("BookController_buyChapter:chapterId或userId为空");
            sender.fail(ErrorCodeEnum.ERROR_CODE_10002.getErrorCode(),
                    ErrorCodeEnum.ERROR_CODE_10002.getErrorMessage(), response);
            return;
        }
        try{
            Chapter chapter = this.chapterService.getChapterById(Long.parseLong(chapterId),0);
            UserAccount userAccount = this.userAccountService.findUniqueByParams("userId",userId);
    ...
    public void toBuyBatchChapter(HttpServletResponse response, HttpServletRequest request) {
        ...
        try{
            Chapter chapter = BookController.this.chapterService.getChapterById(Long.parseLong(chapterId), 0);
    ...
    public void buyBatchChapter(HttpServletResponse response, HttpServletRequest request) {
        ...
        if(StringUtils.isBlank(chapterId) || StringUtils.isBlank(userId)|| StringUtils.isBlank(count)){
            logger.error("BookController_buyBatchChapter:chapterId或userId或count为空");
            sender.fail(ErrorCodeEnum.ERROR_CODE_10002.getErrorCode(),
                    ErrorCodeEnum.ERROR_CODE_10002.getErrorMessage(), response);
            return;
        }
